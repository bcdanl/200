---
title: Classwork 10
subtitle: Working on variables with `select()`, `rename()`, and `relocate()`; Adding a new variable with `mutate()`
date: 2024-04-16
execute: 
  echo: true
  warning: false
...
# Question 1. `select()`; `rename()`; and `relocate()`
...
## Q2f.
- Use the `gapminder` data.frame for Q2f:
```{r}
#| echo: true
#| eval: false
...
## Q2g.
- Use the `gapminder` data.frame for Q2g.
- Use the `gapminder` data.frame for Q2f:
```{r}
#| echo: true
#| eval: false
